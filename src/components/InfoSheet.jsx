...
import "./../assets/css/Header.css"; // Bu CSS faylını növbəti addımda yaradacağıq
...
const InfoSheet = ({ show, onHide }) => {
  ...
  const handleCopyClick = () => {
    navigator.clipboard.writeText(wifiPassword).then(() => {
      setIsCopied(true);
      setTimeout(() => setIsCopied(false), 2000); // 2 saniyə sonra "Kopyalandı" statusunu sıfırla
    });
  };
    ...
    >
      <Offcanvas.Header className="border-0 pb-0">
        <div className="info-sheet-header">
          <Offcanvas.Title as="h5">Baku Cafe Lounge</Offcanvas.Title>
          <p className="text-muted mb-0">Restoran</p>
        </div>
        {/* Xüsusi bağlanma düyməsi */}
